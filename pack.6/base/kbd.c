...
#ifdef DEBUG
volatile struct kthread *ttt;	// Keyboard interrupt handler
#endif
...
void syscall_lscr(void) 
{
	...
	p1 = (struct process *)used_processes;
	while (p1 != NULL) {
		printk("%hd\t%s\n", p1->proc_id, p1->proc_name);
		p1 = (struct process *)p1->proc_next;
	}
	unlock(&kbd_wait_lock);
}

int syscall_gotoscr(int scr) {
	struct process *p1;
	int retval = -1;
	...
	p1 = (struct process *)used_processes;
	while (p1 != NULL) {
		if (p1->proc_id == scr) break;
		else p1 = (struct process *)p1->proc_next;
	}
...
static void display_thread_status(void)
{
	...
	printk("\ntx_wait_pkt : %x, nicwait_start: %d , secs : %d, nic->tx_frame : %x ",tx_wait_pkt, nicwait_start, secs, nic.tx_frame);
}

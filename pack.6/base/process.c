...
int exectask_internal(char pathname[], char *argv[], char *envp[], int type, int local, void *(* start_routine)(void *), void *arg, unsigned int dssize, unsigned int stack_start, unsigned int initial_stacksize, int pno, int thrno, int srcmachine, int mctype, int commslot)
{
	...
	vm_getptentry(current_process->proc_vm, (((unsigned int)0x200050ed) >> 12) & 0x000fffff);
	//pde = vm_getpdentry(current_process->proc_vm, (((unsigned int)0x200050ed) >> 12) & 0x000fffff);
	vm_getpdentry(current_process->proc_vm, (((unsigned int)0x200050ed) >> 12) & 0x000fffff);
...
/*
	...
	printk("exectask_internal: %s -- sections %d\n", pathname, ehdr.e_shnum);
	if (ehdr.e_shnum > 30)
		printk("Error : More number of sections than supported in executable file: %d\n", ehdr.e_shnum);
	// Read all headers limited to a miximum of 30
	for (i=0, hdrcount=0; i<ehdr.e_shnum && hdrcount < 30; i++)
	{
		syscall_read(fd, &hdrs[hdrcount], sizeof(shdr));
		printk("hdr %d type : %d, flags : %d size : %d\n", hdrcount, hdrs[hdrcount].sh_type, hdrs[hdrcount].sh_flags, hdrs[hdrcount].sh_size);
	...
	{
		//printk("hdr %d type : %d, flags : %d\n", i, hdrs[i].sh_type, hdrs[i].sh_flags);
		if (hdrs[i].sh_type == 1)
		{
...
*/
	...
	seg_sizes[0] = seg_sizes[1] = seg_sizes[2] = 0;
	
	syscall_lseek(fd, ehdr.e_phoff, SEEK_SET);	
	if (ehdr.e_phnum > 3) {
		printk("Error : More number of segments than supported in executable file: %d\n", ehdr.e_phnum);
	}

	// Read all headers limited to a miximum of 30
	for (i=0, hdrcount=0; i<ehdr.e_phnum && hdrcount < 3; i++)
	{
		syscall_read(fd, &phdrs[hdrcount], sizeof(Elf32_Phdr));
		//printk("hdr %d type : %d, flags : %d size : f %d, m %d\n", 
			//hdrcount, phdrs[hdrcount].p_type, 
			//phdrs[hdrcount].p_flags, phdrs[hdrcount].p_filesz, 
			//phdrs[hdrcount].p_memsz);

		seg_sizes[hdrcount] = phdrs[hdrcount].p_memsz;
		//if (hdrs[hdrcount].sh_flags != 0)
			hdrcount++;
		// Otherwise discard
	}
	...
	err = vm_addregion(vmem, SEGMENTTYPE_CODE, USERCODE_SEGMENT_START, USERCODE_SEGMENT_START + seg_sizes[0] - 1, 1, 1);
	...
	{
		if (local == 1)
			err = vm_addregion(vmem, SEGMENTTYPE_DATA, USERDATA_SEGMENT_START, USERDATA_SEGMENT_START + seg_sizes[1] - 1, 1, 1);
		else
			err = vm_addregion(vmem, SEGMENTTYPE_DATA, USERDATA_SEGMENT_START, USERDATA_SEGMENT_START + seg_sizes[1] - 1, 1, 0);
	}
...
*/
	...
	for (i=0; i<hdrcount; i++)
	{
		...
		if (((local == 1) && (phdrs[i].p_type == PT_LOAD)) || 
			((local == 0) && (phdrs[i].p_type == PT_LOAD) && (phdrs[i].p_flags & 0x1)))
		{
			syscall_lseek(fd, phdrs[i].p_offset, SEEK_SET); 
			startaddr = phdrs[i].p_vaddr;
			totalsizeread = 0;
			while (totalsizeread < phdrs[i].p_filesz) // Until code section completed
			{
				phys_addr = (unsigned long)logical2physical(vmem->vm_pgdir, (char *)startaddr);
				rounded_addr = (phys_addr + 4096) & 0xfffff000;
				space_inframe = rounded_addr - phys_addr;
		
				rdsize = ((phdrs[i].p_filesz - totalsizeread) > space_inframe) ? space_inframe : (phdrs[i].p_filesz - totalsizeread);
			...
			// Fill zeros in the remaining part of the segment (bss etc.)
			while (totalsizeread < phdrs[i].p_memsz)
			{
				phys_addr = (unsigned long)logical2physical(vmem->vm_pgdir, (char *)startaddr);
				rounded_addr = (phys_addr + 4096) & 0xfffff000;
				space_inframe = rounded_addr - phys_addr;
				rdsize = ((phdrs[i].p_memsz - totalsizeread) > space_inframe) ? space_inframe : (phdrs[i].p_memsz - totalsizeread);
	...
	tp.cs_size = PAGE_ROUND_UP(seg_sizes[0]);
	tp.ds_start = USERDATA_SEGMENT_START;
	tp.ds_size = PAGE_ROUND_UP(seg_sizes[1]);
...
int syscall_pageinfo(int pid, unsigned int pageno)
{
	...
	printk("pid %d, pgno %u, present %d, write %d, pgaddr %x\n", pid, pageno, pdpresent, pdwrite, p);
	restore_paging(old_cr0, current_thread->kt_tss->cr3);
	return 0;
}

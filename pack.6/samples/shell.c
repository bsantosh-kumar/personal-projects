...
int gotoscreen(int argc, char argv[][500]);
int listscr(int argc, char argv[][500]);
...
int split(int argc,char argv[][500])
{
	int count,i,len,fd,fdnew,size=0;
	int sizeread=0;
	char name[15];
	int c=0;
	char buffer[512];
...
int help(int argc, char argv[][500])
{
	...
	if (argc == 2) {
		for (k=0;k<no_cmds;k++)
			if (strcmp(funcnames[k], argv[1]) == 0)
				break;
		cmd = k;
	}
	else cmd = -1; // All
	...
	if (cmd == 20) {
		printf("mem:    Print available and used memory blocks information\n");
		printf("        Usage: mem\n");
	}

	if (cmd == 21) {
		printf("prelease:    Wait for the termination of processes and releases the proc structures\n");
		printf("        Usage: prelease <pid1> <pid2> ...\n");
	}
	if (cmd == 22) {
		printf("pginfo:    Print information about the pages of a give process \n");
		printf("        Usage: pginfo <pid> <pagefrom> <[pageto]>\n");
	}
	if (cmd == 23) {
		printf("id:    Print machine identifier and its MAC address\n");
		printf("        Usage: id\n");
	}
	if (cmd == 24) {
	...
	if (cmd == 25) {
		printf("lscr:    Print list of screens and processes running\n");
		printf("        Usage: lscr\n");
	}
	if (cmd == 26) {
		printf("gotoscr:    Switch to the given screen number \n");
		printf("        Usage: gotoscr <scrnum>\n");
	}

...
void *simulateProcess(void *_a)
{
    ...
    sem_wait(&currProcess->processSem);
    for (int i = 0; i < N; i++)
        sum += currProcess->process_resources->needRes[i];
    sem_post(&currProcess->processSem);
    while (sum != 0)
    {
        sum = 0;
...
void *simulate_kernel(void *_a)
{
    ...
    do
    {
        ...
        for (int i = 0; i < M; i++)
        {
            ...
            /*
                This proces is termianted if <0
            */
            if (allProcesses[i]->request->isRequesting < 0)
            {
                if (allProcesses[i]->request->isRequesting == -1)
                { // Process terminated but not taken resources
                    sem_wait(&printLock);
            ...
            /*
                we have terminate as false, because there is one process still running
            */
            terminate = false;
            /*
                =0 means it is not requesting, but the process is present
            */
            if (allProcesses[i]->request->isRequesting == 0)
            {
                sem_post(&allProcesses[i]->processSem);
            ...
            /*
                =1 means the process is requesting
            */
            else if (allProcesses[i]->request->isRequesting == 1)
            {
                /*
                    First check if the request is less than the need,
                    else we need to terminate the process
                */
                ...
                /*
                    Next check if we have required resources to allocate,
                    if not present then just check for the next process, after releasing the lock
                */
                ...
                /*
                    If we have the required resources, we need to check for safety of the algorithm
                    for that, for temporarily
                    we need to subtract the request from the need of the process,
                    and also from the resources that we have,
                    Then we need to add it to, allocatedResources of the process
                */
                ...
                /*
                    Then release the lock and run
                */
                sem_post(&allProcesses[i]->processSem);

                bool isSafe = safetyAlgorithm(allProcesses, kernelResources);
                sem_wait(&allProcesses[i]->processSem);
                /*
                    If it is safe, then we need to change the isRequesting flag
                    and wake up the process, by resourceSem, sem_post
                */
                if (isSafe)
                {
                    allProcesses[i]->request->isRequesting = 0;
                    printAllocatingResources(allProcesses[i]); // extract
                    sem_wait(&printLock);
                    printf("Kernel resources\n");
                    for (int j = 0; j < N; j++)
                    {
                ...
                /*
                    If not safe, then just revert back the temporary changes that we did
                */
                else
                {
        ...
        /*
            This while loop will end, when all processes are terminated
        */
    } while (!terminate);
}
